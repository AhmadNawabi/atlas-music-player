...
export default function VolumeControls({ volume, setVolume }: VolumeControlsProps) {
  ...
  const toggleMute = () => {
    setVolume(muted ? 0.5 : 0); // restore 50% if unmuting
  };
  ...
  return (
    <div className="mt-2 flex w-64 items-center gap-3">
      ...
      >
        {muted ? (
          <VolumeOff strokeWidth={0.5} className="text-textMuted dark:text-textMuted-dark" />
        ) : (
          <Volume2 strokeWidth={0.5} className="text-text dark:text-text-dark" />
        )}
      </button>
      ...
      <input
        ...
        } bg-gradient-to-r from-gray-300 via-gray-400 to-gray-500 accent-gray-700 transition-all duration-300`}
      />
    </div>
  );
}
